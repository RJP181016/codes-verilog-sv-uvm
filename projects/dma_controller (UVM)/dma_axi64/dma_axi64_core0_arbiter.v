...
module dma_axi64_core0_arbiter(clk,reset,enable,joint_mode,page_cross,joint_req,prio_top,prio_high,prio_top_num,prio_high_num,hold,ch_ready,ch_active,finish,ch_go_out,ch_num,ch_last);
   ...
   //ADDED LOGIC : We are assuming that CH0 > CH1 > CH2 ... > CH7
   assign             ch_num    = ch_num_reg[2:0];
   always @(posedge clk) begin
   		if (ch_ready[0] && ch_active[0]) 	  ch_num_reg = 0;
		else if (ch_ready[1] && ch_active[1]) ch_num_reg = 1;
		else if (ch_ready[2] && ch_active[2]) ch_num_reg = 2;
		else if (ch_ready[3] && ch_active[3]) ch_num_reg = 3;
		else if (ch_ready[4] && ch_active[4]) ch_num_reg = 4;
		else if (ch_ready[5] && ch_active[5]) ch_num_reg = 5;
		else if (ch_ready[6] && ch_active[6]) ch_num_reg = 6;
		else if (ch_ready[7] && ch_active[7]) ch_num_reg = 7;
		else ch_num_reg = 8; //no channel is valid
   end
