...
`include "dma_axi64_ch_reg_params.v"
  ...
  parameter     INT_NUM = 13; 
   ...
   wire              wr_cmd_line2;
   wire              wr_cmd_line3;
   wire              wr_static_line0; //this si for static_reg0
   ...
   assign             wr_static_line3   = gpwrite & gpaddr == STATIC_LINE3; //they are not updating the fields
     ...
     if (reset)
       begin
            rd_burst_max_size_reg <= #1 'd0;
   			rd_tokens_reg         <= #1 'd1; 
    		rd_outs_max_reg       <= #1 {`OUT_BITS{1'b0}}; 	//4'b0100; //3'b100
            rd_incr_reg           <= #1 'd1;
       end
  ...
  wr_tokens_reg         <= #1 pwdata[`TOKEN_BITS+16-1:16]; 
   ...
   assign wr_outstanding_cfg = 1'b0;	//1'b0;
  ...
  assign allow_full_burst   = allow_joint_burst; 
   ...
   always @(allow_full_burst or allow_full_fifo
    ...
    rd_cmd_outs[`OUT_BITS-1:0]            = rd_outs;		//spec: 6bits, design:4bits
    rd_cmd_outs[`OUT_BITS-1+8:8]          = wr_outs;
	//$display("#############################################   DUT:   rd_outs = %b, wr_outs = %b",rd_outs,wr_outs);
